...
function get_orders($baza, $logged_user) {
  $sql = "SELECT Ilosc, Suma, UzytkownikID, KwotaCalkowita, Status, zamowienia.DataUtworzenia, DataAktualizacji, szczegolyzamowienia.PizzaID, Nazwa, Cena, Rozmiar FROM `szczegolyzamowienia` 
  JOIN zamowienia ON szczegolyzamowienia.ZamowienieID = zamowienia.ZamowienieID JOIN pizze ON szczegolyzamowienia.PizzaID = pizze.PizzaID WHERE UzytkownikID = '$logged_user';";
  $orders = mysqli_query($baza, $sql);
  // Tworzenie tabeli dla każdego zamówienia osobno
  foreach($orders as $order) {
    echo "<table class='table table-bordered'>";
    echo "<tr><td>Nazwa:<td/><td>".$order["Nazwa"]."</td></tr>";
    echo "<tr><td>Rozmiar:<td/><td>".$order["Rozmiar"]."</td></tr>";
    echo "<tr><td>Cena:<td/><td>".$order["Cena"]."</td><td>Kwota całkowita:</td></tr>";
    echo "<tr><td>Ilość:<td/><td>".$order["Ilosc"]."</td><td>".$order["KwotaCalkowita"]."</td></tr>";
...
<main>
  <div class="col-8">
    <?php get_orders($baza, $logged_user); ?>
  </div>
</main>
